...
<template>
  <div class="px-3 mb-4">
    <!-- Section Title -->
    <h3 class="mb-2 text-[10px] font-medium text-slate-400 uppercase tracking-wider px-0 flex items-center justify-between">
      <span>Team</span>
      <span class="text-slate-300">{{ otherMembers.filter(m => m.isFocused).length }}</span>
    </h3>
    ...
    <div class="space-y-0.5">
      <div
        ...
        :class="member.isFocused ? 'text-slate-700' : 'text-slate-400'"
      >
        <!-- Avatar -->
        <div class="relative flex-shrink-0">
          <div 
            class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-medium"
            :class="member.avatar ? '' : 'bg-slate-200 text-slate-600'"
          >
            <img 
              v-if="member.avatar" 
              :src="member.avatar" 
          ...
          <span 
            ...
            class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border-2 border-white bg-emerald-400"
          ></span>
        </div>
        ...
        <div 
          v-if="member.isFocused"
          class="w-4 h-4 flex items-center justify-center text-slate-400 flex-shrink-0"
        >
          <Icon :name="getActivityIcon(member)" class="w-3.5 h-3.5" />
        </div>
      </div>

      <div v-if="!otherMembers.length" class="px-3 py-2 text-[10px] text-slate-400 italic">
        No team members
      </div>
    </div>
  </div>
