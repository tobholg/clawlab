...
// Navigate to parent or child within panel (in-place)
...
// Fetch when itemId changes (panel mounts with an item to display)
...
watch(
  [() => props.itemId, () => itemDetail.value?.planDocId, hasAgentAssignee],
  ([id, planDocId, hasAgent]) => {
    if (!id || !hasAgent || !planDocId) {
      planDocument.value = null
      planDocumentError.value = null
      return
    }
...
// Fetch available users when itemId changes
...
// Reset tracking state when item unmounts
onUnmounted(() => {
...
const handleClose = async () => {
  ...
  emit('update', currentItemId.value ?? props.itemId ?? '', { _close: true })
  emit('close')
}
  ...
  <div class="relative bg-white dark:bg-dm-panel w-full h-full flex flex-col">
          ...
          <div class="px-6 py-4 border-b border-slate-100 dark:border-white/[0.06] flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <!-- Focus Button -->
              <button
                v-if="!isFocusedOnTask(itemId)"
                @click="startTaskFocus(itemId)"
