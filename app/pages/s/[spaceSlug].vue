    ...
    <nav class="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100">
      <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <div class="flex items-center gap-4">
          <NuxtLink to="/dashboard" class="flex items-center gap-2 text-slate-400 hover:text-slate-600 transition-colors group">
          ...
          <div v-if="space" class="flex items-center gap-3">
            <div>
              <span class="font-semibold text-slate-900">{{ space.name }}</span>
              <span class="text-slate-300 mx-2">·</span>
              <span class="text-sm text-slate-500">Stakeholder Portal</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-full border border-slate-100">
            ...
            <span class="text-sm font-medium text-slate-700">{{ space?.access?.displayName || user?.name || user?.email }}</span>
          </div>
          <button
            @click="logout"
            class="text-sm text-slate-400 hover:text-slate-600 transition-colors px-3 py-1.5"
          >
            Sign out
          </button>
        </div>
    ...
    <main :class="['pt-16 transition-all duration-300 ease-out', chatOpen ? 'mr-96' : '']">
      ...
      >
        <div v-if="loading" class="flex flex-col items-center justify-center py-32 gap-4">
          <div class="relative">
            <div class="w-12 h-12 rounded-2xl bg-violet-100 animate-pulse" />
            <Icon name="heroicons:arrow-path" class="absolute inset-0 m-auto w-6 h-6 text-violet-500 animate-spin" />
          </div>
          <p class="text-sm text-slate-400">Loading portal...</p>
        </div>
      </Transition>
      ...
      >
        <div v-if="error && !loading" class="max-w-lg mx-auto mt-32 px-6">
          <div class="bg-white border border-slate-100 rounded-2xl p-10 text-center shadow-xl shadow-slate-100">
            <div class="w-16 h-16 mx-auto mb-6 bg-rose-50 rounded-2xl flex items-center justify-center">
              <Icon name="heroicons:exclamation-circle" class="w-8 h-8 text-rose-400" />
            </div>
            <h2 class="text-xl font-semibold text-slate-900 mb-2">Access Denied</h2>
            <p class="text-slate-500 mb-8">{{ error }}</p>
            <NuxtLink 
              to="/dashboard"
              class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white text-sm font-medium rounded-xl hover:bg-slate-800 transition-colors"
            >
              <Icon name="heroicons:arrow-left" class="w-4 h-4" />
              Back to My Portals
            </NuxtLink>
      ...
      >
        <div v-if="space && !loading && !error">
          ...
          <div class="relative overflow-hidden">
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-violet-50/50 via-transparent to-transparent pointer-events-none" />
            ...
            <div class="relative max-w-6xl mx-auto px-6 pt-10 pb-6">
              ...
              >
                <div class="flex items-start justify-between gap-6 mb-8">
                  <div class="flex-1">
                    <h1 class="text-3xl font-semibold text-slate-900 mb-2 tracking-tight">{{ space.project.title }}</h1>
                    <p v-if="space.project.description" class="text-slate-500 text-lg max-w-2xl leading-relaxed">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div class="flex -space-x-2">
                        <div 
                          ...
                          :key="stakeholder.id"
                          class="w-8 h-8 bg-gradient-to-br from-violet-400 to-violet-500 rounded-full flex items-center justify-center ring-2 ring-white"
                        >
                        ...
                        <div 
                          ...
                          class="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center ring-2 ring-white"
                        >
                          <span class="text-xs font-medium text-slate-500">+{{ space.stakeholderCount - 3 }}</span>
                        </div>
                      </div>
                      <div class="flex items-center gap-1 text-sm font-medium text-slate-700 group-hover:text-violet-600 transition-colors">
                        <span>{{ space.stakeholderCount }} {{ space.stakeholderCount === 1 ? 'person' : 'people' }}</span>
                        <Icon name="heroicons:chevron-right" class="w-4 h-4 text-slate-400 group-hover:text-violet-500 transition-colors" />
                      </div>
                    </div>
                  </button>
                </div>
                ...
                <!-- Your role badge -->
                <div v-if="space.access.position" class="inline-flex items-center gap-2 px-3 py-1.5 bg-violet-50 rounded-full border border-violet-100">
                  <Icon name="heroicons:user-circle" class="w-4 h-4 text-violet-500" />
                  <span class="text-sm font-medium text-violet-700">{{ space.access.position }}</span>
                </div>
              </div>
            </div>
          ...
          <div class="max-w-6xl mx-auto px-6 py-8">
            ...
            >
                  ...
                  <div v-if="space.stats" class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex items-center gap-4">
                      <div class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0">
                        <Icon name="heroicons:rectangle-stack" class="w-6 h-6 text-blue-500" />
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-slate-900">{{ space.stats.totalActive }}</div>
                        <div class="text-xs text-slate-500 font-medium">Active Items</div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex items-center gap-4">
                      <div class="w-12 h-12 bg-emerald-50 rounded-xl flex items-center justify-center flex-shrink-0">
                        <Icon name="heroicons:check-circle" class="w-6 h-6 text-emerald-500" />
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-slate-900">{{ space.stats.totalCompleted }}</div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex items-center gap-4">
                      <div class="w-12 h-12 bg-violet-50 rounded-xl flex items-center justify-center flex-shrink-0">
                        <Icon name="heroicons:chart-bar" class="w-6 h-6 text-violet-500" />
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-slate-900">{{ space.stats.avgProgress }}%</div>
                        <div class="text-xs text-slate-500 font-medium">Avg Progress</div>
                      </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex items-center gap-4">
                      <div class="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center flex-shrink-0">
                        <Icon name="heroicons:signal" class="w-6 h-6 text-amber-500" />
                      </div>
                      <div>
                        <div class="text-2xl font-bold text-slate-900">{{ space.stats.avgConfidence }}%</div>
                        <div class="text-xs text-slate-500 font-medium">Avg Confidence</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div v-else-if="activeTab === 'requests'" key="requests">
                ...
                <div v-else-if="!myRequests || (myRequests.tasks.length === 0 && myRequests.irs.length === 0)" class="bg-white border border-slate-100 rounded-2xl p-16 text-center shadow-sm">
                  ...
                  <h3 class="text-lg font-semibold text-slate-900 mb-2">No requests yet</h3>
                  <p class="text-slate-500 max-w-sm mx-auto">Your submitted tasks and questions will appear here. Use the chat to submit new requests.</p>
                </div>
                ...
                <div v-else class="space-y-8">
                  ...
                  <section v-if="myRequests.tasks.length > 0">
                    <div class="flex items-center gap-3 mb-4">
                      <h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wider">Submitted Tasks</h2>
                    ...
                    <div class="space-y-4">
                      ...
                      >
                        ...
                        <div v-if="task.status === 'rejected' && task.rejectionReason" class="mt-3 p-3 bg-rose-50 border border-rose-100 rounded-xl">
                          <p class="text-xs text-rose-600 font-medium mb-1">Reason for decline:</p>
                          <p class="text-sm text-rose-800">{{ task.rejectionReason }}</p>
                        </div>
                        
                        <div v-if="task.status === 'accepted'" class="mt-3 p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-100">
                          <p class="text-xs text-slate-400 font-medium mb-3 uppercase tracking-wider">Conversation</p>
                          ...
                          <div v-if="task.comments && task.comments.length > 0" class="space-y-2 mb-4">
                            ...
                            >
                              <div class="flex items-center gap-2 mb-1">
                                <span :class="['text-xs font-semibold', comment.isTeamMember ? 'text-violet-700' : 'text-slate-700']">
                                  {{ comment.authorName }}
                                </span>
                                <span v-if="comment.isTeamMember" class="text-[10px] px-1.5 py-0.5 bg-violet-200 text-violet-700 rounded-full font-medium">
                                  Team
                                </span>
                          ...
                          <div v-else class="text-xs text-slate-400 mb-4 py-4 text-center bg-slate-50 rounded-xl">
                            No messages yet
                          </div>
                          ...
                          <div class="flex gap-2">
                            <textarea
                              ...
                              class="flex-1 px-4 py-3 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none bg-slate-50"
                            />
                            <button
                        ...
                        <p class="text-xs text-slate-400 mt-4">Submitted {{ formatDate(task.createdAt) }}</p>
                      </div>
                    </div>
                  </section>
                  ...
                  <section v-if="myRequests.irs.length > 0">
                    <div class="flex items-center gap-3 mb-4">
                      <h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wider">My Questions & Suggestions</h2>
                      <div class="h-px flex-1 bg-gradient-to-r from-slate-200 to-transparent" />
                    </div>
                    ...
                    <div class="space-y-4">
                      ...
                      >
                        <div class="flex items-start gap-4 mb-3">
                          <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                              <span class="text-xs text-slate-400 uppercase font-medium">{{ ir.type }}</span>
                              <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', statusColor(ir.status)]">
                                {{ ir.status }}
                              </span>
                            </div>
                            <p class="text-slate-900">{{ ir.content }}</p>
                            
                            <!-- Official Response -->
                            <div v-if="ir.response" class="mt-4 p-4 bg-violet-50 border border-violet-100 rounded-xl">
                              <p class="text-xs text-violet-600 font-semibold mb-2 uppercase tracking-wider">Official Response</p>
                              <p class="text-sm text-slate-700">{{ ir.response }}</p>
                            </div>
                        ...
                        <p class="text-xs text-slate-400">Asked {{ formatDate(ir.createdAt) }}</p>
                      </div>
                    </div>
                  </section>
                </div>
              ...
              <div v-else-if="activeTab === 'qa'" key="qa">
                ...
                <div class="flex items-center gap-4 mb-6">
                  <div class="relative flex-1 max-w-md">
                    <Icon name="heroicons:magnifying-glass" class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input
                      v-model="qaSearch"
                      type="text"
                      placeholder="Search questions..."
                      class="w-full pl-11 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                    />
                  </div>
                  <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
                    <button
                      ...
                      :class="[
                        ...
                        qaSort === 'recent' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                      ]"
                    >
                      Recent
                    </button>
                    <button
                      ...
                      :class="[
                        'px-3 py-1.5 text-sm font-medium rounded-md transition-all',
                        qaSort === 'votes' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                      ]"
                    >
                      Most Voted
                    </button>
                ...
                <div v-if="loadingQA" class="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p class="text-sm text-slate-400">Loading Q&A...</p>
                </div>
                
                <div v-else-if="qaItems.length === 0" class="bg-white border border-slate-100 rounded-2xl p-16 text-center shadow-sm">
                  <div class="w-20 h-20 mx-auto mb-6 bg-slate-50 rounded-3xl flex items-center justify-center">
                    <Icon name="heroicons:chat-bubble-left-right" class="w-10 h-10 text-slate-300" />
                  </div>
                  <h3 class="text-lg font-semibold text-slate-900 mb-2">No questions yet</h3>
                  <p class="text-slate-500 max-w-sm mx-auto">Be the first to ask a question! Use the chat to submit your questions.</p>
                </div>
                  ...
                  >
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-2">
                          <span class="text-xs text-slate-400 uppercase font-medium">{{ item.type }}</span>
                          <span :class="['text-xs font-medium px-2.5 py-1 rounded-full', statusColor(item.status)]">
                            {{ item.status }}
                          </span>
                        </div>
                        <p class="text-slate-900 text-lg">{{ item.content }}</p>
                        
                        <!-- Official Response -->
                        <div v-if="item.response" class="mt-4 p-4 bg-violet-50 border border-violet-100 rounded-xl">
                          <p class="text-xs text-violet-600 font-semibold mb-2 uppercase tracking-wider">Official Response</p>
                          <p class="text-sm text-slate-700">{{ item.response }}</p>
                        </div>
                        
                        <!-- Comments Thread -->
                        <div v-if="item.comments && item.comments.length > 0" class="mt-4 space-y-2">
                          <p class="text-xs text-slate-400 font-medium uppercase tracking-wider">Discussion</p>
                          <div
            ...
            >
              ...
              <div class="px-6 py-5 border-b border-slate-100">
                <div class="flex items-center justify-between">
                  <div>
                    <h2 class="text-lg font-semibold text-slate-900">Stakeholders</h2>
                    <p class="text-sm text-slate-500 mt-0.5">{{ space.stakeholderCount }} {{ space.stakeholderCount === 1 ? 'person has' : 'people have' }} access to this portal</p>
                  </div>
                  <button 
              ...
              <div class="px-6 py-4 max-h-[400px] overflow-y-auto">
                <div class="space-y-3">
                  ...
                  >
                    <!-- Avatar -->
                    <div class="w-10 h-10 bg-gradient-to-br from-violet-400 to-violet-500 rounded-xl flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <span v-if="stakeholder.position" class="text-slate-300">·</span>
                ...
                <div v-if="space.stakeholders.length === 0" class="text-center py-8">
                  <div class="w-12 h-12 mx-auto mb-3 bg-slate-100 rounded-xl flex items-center justify-center">
                    <Icon name="heroicons:users" class="w-6 h-6 text-slate-400" />
                  </div>
                  <p class="text-sm text-slate-500">No stakeholders yet</p>
                </div>
              </div>
            </div>
          </Transition>
