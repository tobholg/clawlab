  ...
  <div class="flex h-screen bg-[#FAFAFA] font-sans text-slate-900 overflow-hidden">
        ...
        <div class="space-y-0.5">
          <button
            ...
            :class="[
              ...
              isRootLevel && activeView === 'dashboard'
          ...
          <button
            @click="navigateTo('root'); activeView = 'timeline'"
            :class="[
              'w-full flex items-center rounded-lg text-sm font-normal transition-all duration-200',
              isRootLevel && activeView === 'timeline'
                ? 'text-slate-900 bg-slate-50' 
                : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50',
              sidebarCollapsed ? 'justify-center px-2 py-2' : 'gap-2.5 px-3 py-2'
            ]"
            :title="sidebarCollapsed ? 'Timeline' : undefined"
          >
            <Icon name="heroicons:calendar" class="w-4 h-4 flex-shrink-0" />
            <span :class="['transition-all duration-300 overflow-hidden whitespace-nowrap', sidebarCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100']">Timeline</span>
          </button>
          
          <NuxtLink
            to="/dependencies"
            :class="[
              'flex items-center rounded-lg text-sm font-normal text-slate-500 hover:text-slate-700 hover:bg-slate-50 transition-all duration-200',
              sidebarCollapsed ? 'justify-center px-2 py-2' : 'gap-2.5 px-3 py-2'
            ]"
            :title="sidebarCollapsed ? 'Dependencies' : undefined"
          >
            <Icon name="heroicons:arrow-path" class="w-4 h-4 flex-shrink-0" />
            <span :class="['transition-all duration-300 overflow-hidden whitespace-nowrap', sidebarCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100']">Dependencies</span>
          </NuxtLink>
        </div>
    ...
    <main class="flex-1 flex flex-col min-w-0 relative overflow-hidden">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                class="text-xl font-medium text-slate-900 bg-transparent border-0 focus:outline-none focus:ring-0 placeholder-slate-300 p-0"
                placeholder="Project title..."
              />
              <h1 v-else class="text-xl font-medium text-slate-900">Projects</h1>
              
              <!-- Editable description when inside project -->
              <input
                v-if="!isRootLevel"
                v-model="editedProjectDescription"
                type="text"
                class="text-sm text-slate-400 bg-transparent border-0 focus:outline-none focus:ring-0 placeholder-slate-400 p-0 mt-0.5 w-full"
      ...
      <div class="flex-1 overflow-auto px-6 pb-8">
        ...
        <!-- List View (inside project) -->
        <div v-else-if="!isRootLevel && activeView === 'list'" class="text-sm text-slate-400">
